...
export function DashboardTopBar() {
  ...
  const handleNewNotification = useCallback((n: CommsNotification) => {
    console.log('[comms-notify][topbar] queueing', { id: n.id });
    setNotifications((prev) => {
      // Cap at 5 stacked toasts; newest first
      const next = [n, ...prev.filter((x) => x.id !== n.id)].slice(0, 5);
      console.log('[comms-notify][topbar] queue size now', next.length);
      return next;
    });
  }, []);
  ...
  const handleDismiss = useCallback((id: string) => {
    console.log('[comms-notify][topbar] dismissing', { id });
    setNotifications((prev) => prev.filter((n) => n.id !== id));
  }, []);

  // Subscribe to SSE stream
  useCommsSSE(handleNewNotification);
  ...
  return (
    <>
      ...
      {/* Toast notification stack — fixed, top-right under topbar */}
      <div
        className="fixed top-[72px] right-4 z-[60] flex flex-col gap-2 pointer-events-none"
